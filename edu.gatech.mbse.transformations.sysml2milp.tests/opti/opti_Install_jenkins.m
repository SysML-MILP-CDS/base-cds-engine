...
%   Slightly modified by Sebastian Herzig so that it can be used as part of
%   an automated script (got rid of any required user feedback).

if exist('isSetup', 'var') == 1
    fprintf('\nOPTI TOOLBOX is already set up...\n')
...
fprintf('\n------------------------------------------------\n')
fprintf(['  SETTING UP OPTI TOOLBOX ver ' sprintf('%1.2f',cur_ver) '\n\n'])
...
genp = genpath(cd);
genp = regexp(genp,pathsep,'split');
...
rehash
fprintf('Done\n\n');
in = 'n';%input('- Would You Like To Save the Path Changes? (Recommended) (y/n): ','s');
if(strcmpi(in,'y'))
    try
        savepath;
    catch %#ok<CTCH>
